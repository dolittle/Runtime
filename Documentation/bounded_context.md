...
##### Inter-Bounded-Context-Communication; Single source of truth and Events
Maybe write a little summary here?

## The configuration
bounded-context.json that sits inside the .dolittle folder in the folder where you build the Bounded Context (if you have a web interaction layer then that would typically be where you would perform the build):
```json
{
...
* proxiesBasePath - The base path relative to the path where the build is performed from and the .dolittle folder is where the proxies will be created.
* namespaceSegmentsToStrip - I dictionary of namespace segments to strip from the namespace when creating the Artifacts when the first segment of the namespace is equal to the Key.
* topology: The topology structure object. This is automagically created when you build the Bounded Context, given that you have already provided the other information described above to the configuration file.
