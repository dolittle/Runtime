...
namespace Integration.Tests.Events.Processing.EventHandlers.with_a_single.scoped.not_partitioned.given;

class single_tenant_and_event_handlers : scoped.given.single_tenant_and_event_handlers
{
    ...
    protected static void with_event_handlers_filtering_number_of_event_types(params (int max_event_types_to_filter, bool fast, bool implicitFilter)[] event_handler_infos)
        => with_event_handlers_filtering_number_of_event_types(event_handler_infos
            .Select(_ => (false, _.max_event_types_to_filter, event_handler_scope, _.fast, _.implicitFilter))
            .ToArray());
