...
namespace Integration.Tests.Events.Processing.EventHandlers.with_a_single.scoped.unpartitioned.fast_event_handler.needing_to_catchup.unprocessing_one_event_type.and_failing;
...
class on_one_partition : given.single_tenant_and_event_handlers
{
    ...
    It should_have_the_correct_stream_processor_states = () => expect_stream_processor_state(
        ...
        failing_unpartitioned_state: new failing_unpartitioned_state(get_partitioned_events_in_stream(event_handler, failing_partition).First().Position));
}