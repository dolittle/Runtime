...
namespace Integration.Tests.Events.Processing.EventHandlers.with_a_single.partitioned.fast_event_handler.needing_to_catchup.processing_all_event_types;
...
//         {
//             (10, "some_source", ScopeId.Default)
...
class after_2_events : given.single_tenant_and_event_handlers
{
    ...
    Establish context = () =>
    {
        ...
        commit_events_for_each_event_type(new (int number_of_events, EventSourceId event_source, ScopeId scope_id)[]
        {
            (10, failing_partition.Value, ScopeId.Default)
        }).GetAwaiter().GetResult();
    ...
    Because of = () =>
    {
        stop_event_handlers_after(TimeSpan.FromSeconds(5));
        run_event_handlers_until_completion_and_commit_events_after_starting_event_handlers().GetAwaiter().GetResult();
    };
