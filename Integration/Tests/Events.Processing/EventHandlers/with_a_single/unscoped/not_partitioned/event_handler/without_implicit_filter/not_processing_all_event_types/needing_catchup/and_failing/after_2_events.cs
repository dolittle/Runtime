...
namespace Integration.Tests.Events.Processing.EventHandlers.with_a_single.unscoped.not_partitioned.event_handler.without_implicit_filter.
    processing_all_event_types.needing_catchup.and_failing;
...
class after_2_events : given.single_tenant_and_event_handlers
{
    ...
    Because of = () => { commit_events_after_starting_event_handler((2, failing_partition.Value)); };

    It should_have_persisted_correct_stream = () => expect_stream_definition(event_handler);

    It should_have_the_correct_stream_processor_states =
        () => expect_stream_processor_state_with_failure(event_handler, new failing_unpartitioned_state(1, failure_reason));
}