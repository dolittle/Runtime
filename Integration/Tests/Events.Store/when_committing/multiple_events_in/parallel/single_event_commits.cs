...
class single_event_commits : given.a_clean_event_store
{
    ...
    class not_for_aggregate
    {
        ...
        Establish context = () =>
        {
            uncommitted_events = Enumerable.Range(0, number_of_parallel_committers).Select(commiter => new UncommittedEvents(
                ...
                {
                    given.event_to_commit.with_content(new {Hello = $"From {commiter}"}).with_event_source($"committer {commiter}").build()
                })).ToArray();
        };
        ...
        Establish context = () =>
        {
            ...
            uncommitted_events = Enumerable.Range(0, number_of_parallel_committers).Select(committer =>
            {
                var events = new UncommittedAggregateEvents(
                    ...
                    new List<UncommittedEvent>
                    {
                        given.event_to_commit.with_content(new {Hello = $"From {committer}"}).with_event_source(event_source).build()
                    });
                return events;
            }).ToArray();
        };
        ...
        Establish context = () =>
        {
            ...
            uncommitted_events = Enumerable.Range(0, number_of_parallel_committers).Select(committer =>
            {
                ...
                var events = new UncommittedAggregateEvents(
                    ...
                    new List<UncommittedEvent>
                    {
                        given.event_to_commit.with_content(new {Hello = $"From aggregate {committer}"}).with_event_source(event_source).build()
                    });
                return events;
            }).ToArray();
        };
    ...
    class for_different_aggregates_and_non_aggregate_events
    {
        ...
        Establish context = () =>
        {
            ...
            non_agggregate_uncommitted_events = Enumerable.Range(0, number_of_parallel_committers).Select(commiter => new UncommittedEvents(
                new List<UncommittedEvent>
                {
                    given.event_to_commit.with_content(new {Hello = $"From {commiter}"}).with_event_source($"committer {commiter}").build()
                })).ToArray();
