...
class specs : given.a_clean_event_store
{
    ...
    static Try<IAsyncEnumerable<CommittedAggregateEvent>> response;
    ...
    class for_a_tenant_that_is_not_configured
    {
        ...
        Because of = () => response = event_store.FetchAggregateEvents(event_source, aggregate_root_id, event_types, "d48ca32c-bc98-4d6e-9e8d-4eaaf5adb579", CancellationToken.None);
    ...
    {
        Because of = () => response = event_store.FetchAggregateEvents(event_source, aggregate_root_id, event_types, execution_context.Tenant, CancellationToken.None);

        It should_fail = () => response.Success.ShouldBeFalse();

        It should_fail_because_cannot_fetch_with_no_event_types = () => response.Exception.ShouldBeOfExactType<CannotFetchAggregateEventsWithNoEventTypes>();
    }
    ...
    class and_there_are_no_events_for_aggregate
    {
        ...
        It should_have_no_aggregate_events = () => to_committed_events(response.Result, uncommitted_events).ShouldBeEmpty();
    }
    ...
    class and_there_is_one_event_for_aggregate_with_wanted_event_type
    {
        ...
        It should_return_the_correct_committed_event = () => should_extensions.events_should_be_the_same(
            to_committed_events(response.Result, uncommitted_events),
            uncommitted_events,
            execution_context,
    ...
    static CommittedAggregateEvents to_committed_events(IAsyncEnumerable<CommittedAggregateEvent> stream, UncommittedAggregateEvents uncommitted_events)
    {
        var events = stream.ToListAsync().Result;
        return new CommittedAggregateEvents(uncommitted_events.EventSource, uncommitted_events.AggregateRoot.Id, events);
    }
