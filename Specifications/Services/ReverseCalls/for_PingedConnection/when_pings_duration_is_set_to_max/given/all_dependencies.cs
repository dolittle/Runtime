...
namespace Dolittle.Runtime.Services.ReverseCalls.for_PingedConnection.when_pings_duration_is_set_to_max.given;

public class all_dependencies : mother_of_all_dependencies
{
    protected static Scenario scenario;
    protected static a_message first_message_with_0_second_pings;

    Establish context = () =>
    {
        first_message_with_0_second_pings = new a_message();
        var first_message_connect_arguments = new object();
        var first_message_arguments_context = new ReverseCallArgumentsContext()
        {
            PingInterval = Duration.FromTimeSpan(TimeSpan.Zero),
        };

        message_converter
            .Setup(_ => _.GetConnectArguments(first_message_with_0_second_pings))
