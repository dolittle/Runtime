...
public class when_failing_on_next_event: state_with_failing_partititons
{
    ...
    {
        after_state = before_state.WithResult(new FailedProcessing(reason), next_event, now);
    }
    ...
    public void ShouldAddNewFailedPartition()
    {
        ...
        failing_partition.Should().BeEquivalentTo(new FailingPartitionState(next_event.CurrentProcessingPosition, DateTimeOffset.MaxValue, reason, 1, now));
