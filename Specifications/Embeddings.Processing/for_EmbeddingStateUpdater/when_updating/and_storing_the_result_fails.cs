...
namespace Dolittle.Runtime.Embeddings.Processing.for_EmbeddingStateUpdater.when_updating
{
    public class and_storing_the_result_fails : given.all_dependencies
    {
        ...
        Establish context = () =>
        {
            ...
            project_many_events.Setup(_ => _.TryProject(current_state, unprocessed_events, cancellation_token)).Returns(Task.FromResult<Partial<EmbeddingCurrentState>>(projection_result));
        };
        ...
        It should_project_the_event = () => project_many_events.Verify(_ => _.TryProject(current_state, unprocessed_events, cancellation_token));
