...
public class and_initial_state_differs : given.one_tenant
{
    ...
    Establish context = () =>
    {
        ...
        dynamic new_state = new JObject();
        persisted_state.Hello = "world";
        new_state.OtherField = "something";
        persisted_definition = new EmbeddingDefinition(
            embedding,
            new[] { new Artifact("0fed7849-59e2-4e32-a9e4-91f3f824c5ac", 1) },
            JsonConvert.SerializeObject(persisted_state));
        new_definition = persisted_definition with { InititalState = JsonConvert.SerializeObject(new_state) };
        definitions
    ...
    It should_not_be_a_successful_result = () => results[tenant].Succeeded.Should().BeFalse();
