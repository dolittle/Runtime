...
public class and_everything_works : given.all_dependencies
{
    ...
    It should_have_the_correct_state = () => result.State.State.Should().Be(state.Value);
    It should_have_the_correct_state_type = () => result.State.Type.Should().Be(Projections.Contracts.ProjectionCurrentStateType.Persisted);
    It should_have_the_correct_state_key = () => result.State.Key.Should().Be(key.Value);
}