...
public class when_starting_two_processors : given.two_tenants_and_processors
{
    ...
    It should_not_create_processors_for_any_other_tenants = () => factory.Verify(_ => _(Moq.It.IsAny<TenantId>()), Times.Exactly(2));
    It should_have_processors_for_the_embedding = () => processors.HasEmbeddingProcessors(embedding).Should().BeTrue();
    It should_return_processor_a_for_tenant_a = () =>
    {
        processors.TryGetEmbeddingProcessorFor(tenant_a, embedding, out var processor).Should().BeTrue();
        processor.ShouldBeTheSameAs(processor_a);
    };
    It should_return_processor_b_for_tenant_b = () =>
    {
        processors.TryGetEmbeddingProcessorFor(tenant_b, embedding, out var processor).Should().BeTrue();
        processor.ShouldBeTheSameAs(processor_b);
    };
    It should_not_have_a_processor_for_another_tenant = () => processors.TryGetEmbeddingProcessorFor("621aca1c-eddf-4ed4-a2ca-512d9a49d746", embedding, out var _).Should().BeFalse();
    It should_not_have_processors_for_another_embedding = () => processors.HasEmbeddingProcessors("f9bd3173-b7a8-4081-ab0d-61351e6a9cb5").Should().BeFalse();
