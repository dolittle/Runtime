...
public class and_everything_works : given.all_dependencies
{
    ...
    It should_return_a_deleted_state = () => result.Result.Type.Should().Be(EmbeddingCurrentStateType.Deleted);
    It should_return_the_same_key = () => result.Result.Key.Should().Be(projection_key);
    It should_return_the_correct_aggregate_root_version = () => result.Result.Version.Value.Should().Be(current_state.Version.Value + 3);
