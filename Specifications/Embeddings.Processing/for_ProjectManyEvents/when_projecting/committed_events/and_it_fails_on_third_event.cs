...
public class and_it_fails_on_third_event : given.all_dependencies
{
    ...
    Establish context = () =>
    {
        ...
        embedding
            .Setup(_ => _.Project(Moq.It.Is<ProjectionCurrentState>(_ => _.State.Value == result_after_two.State.Value), Moq.It.Is<UncommittedEvent>(_ => _.Content == event_three.Content), execution_context, cancellation_token))
    ...
    It should_be_a_partial_success = () => result.IsPartialResult.Should().BeTrue();
    It should_fail_wih_the_correct_exception = () => result.Exception.Should().Be(exception);
    It should_return_the_second_state = () => result.Result.State.Should().Be(result_after_two.State);
    It should_return_a_persisted_result = () => result.Result.Type.Should().Be(EmbeddingCurrentStateType.Persisted);
    It should_return_the_same_key = () => result.Result.Key.Should().Be(projection_key);
    It should_return_the_correct_aggregate_root_version = () => result.Result.Version.Value.Should().Be(current_state.Version.Value + 2);
    ...
    It should_project_the_third_event = () => embedding.Verify(_ => _.Project(Moq.It.Is<ProjectionCurrentState>(_ => _.State.Value == result_after_two.State.Value), Moq.It.Is<UncommittedEvent>(_ => _.Content == event_three.Content), execution_context, cancellation_token));
}