...
namespace Dolittle.Runtime.Embeddings.Processing.for_EmbeddingProcessor.when_updating
{
    public class and_updating_the_state_fails : given.all_dependencies_and_a_desired_state
    {
        ...
        Establish context = () =>
        {
            ...
            embedding_store.Setup(_ => _.TryReplace(embedding, key, Moq.It.IsAny<AggregateRootVersion>(), desired_state, Moq.It.IsAny<CancellationToken>())).Returns(Task.FromResult(Try.Failed(exception)));
        };
