...
namespace Dolittle.Runtime.Events.Store.MongoDB.Processing.Streams.for_StreamProcessorStateRepository.when_getting_state
{
    public class and_state_is_stored : given.all_dependencies
    {
        ...
        Establish context = () =>
        {
            ...
            initial_state = repository.GetOrAddNew(stream_processor_id, CancellationToken.None).GetAwaiter().GetResult();
        };

        Because of = () => result = repository.GetOrAddNew(stream_processor_id, CancellationToken.None).GetAwaiter().GetResult();
