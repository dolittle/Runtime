...
namespace Dolittle.Runtime.Events.Store.MongoDB.Processing.for_EventsToStreamsWriter
{
    public class when_writing_an_event_to_stream : given.all_dependencies
    {
        ...
        It should_have_stored_the_event_with_exactly_the_same_data_as_committed_event = () => committed_event.ShouldBeStoredWithCorrectStoreRepresentation(stream.Find(filters.an_event_filter.Empty).First(), 0, partition);
    }
}