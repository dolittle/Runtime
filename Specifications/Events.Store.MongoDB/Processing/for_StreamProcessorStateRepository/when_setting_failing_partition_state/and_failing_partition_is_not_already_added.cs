...
namespace Dolittle.Runtime.Events.Store.MongoDB.Processing.for_StreamProcessorStateRepository.when_setting_failing_partition_state
{
    public class and_failing_partition_is_not_already_added : given.all_dependencies
    {
        ...
        Because of = () => exception = Catch.Exception(() => repository.SetFailingPartitionState(stream_processor_id, Guid.NewGuid(), new Runtime.Events.Processing.FailingPartitionState()).GetAwaiter().GetResult());

        It should_throw_an_exception = () => exception.ShouldNotBeNull();
        It should_fail_because_failing_partition_does_not_exist = () => exception.ShouldBeOfExactType<Runtime.Events.Processing.FailingPartitionDoesNotExist>();
    }
}