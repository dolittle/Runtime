...
namespace Dolittle.Runtime.Events.Store.MongoDB.Events.for_CommittedEventExtensions.when_converting_to_committed_event
{
    public class from_event_from_event_horizon
    {
        ...
        Establish context = () =>
        {
            @event = events.new_event_not_from_aggregate(0).from_event_horizon().build();
        };
        ...
        It should_have_the_correct_event_log_sequence_number = () => result.EventLogSequenceNumber.Value.ShouldEqual(@event.EventLogSequenceNumber);
