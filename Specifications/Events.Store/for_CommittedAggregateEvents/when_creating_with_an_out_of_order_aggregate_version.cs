...
namespace Dolittle.Runtime.Events.Store.Specs.for_CommittedAggregateEvents
{
    public class when_creating_with_an_out_of_order_aggregate_version : given.events_and_an_artifact
    {
        ...
        Establish context = () =>
        {
            out_of_order_event = new CommittedAggregateEvent(event_source_id, aggregate_artifact, aggregate_version, 3, DateTimeOffset.Now, correlation_id, microservice_id, tenant_id, cause, event_b_artifact, "wrong");
        };

        Because of = () => exception = Catch.Exception(() =>
        {
            events = new CommittedAggregateEvents(event_source_id, aggregate_artifact, aggregate_version, new[] { event_one, event_two, event_three, out_of_order_event });
        });
