...
namespace Dolittle.Runtime.Events.Store.Specs.for_CommittedAggregateEvents
{
    public class when_creating_with_a_wrong_artifact : given.events_and_an_artifact
    {
        ...
        Establish context = () =>
        {
            wrong_aggregate_event = new CommittedAggregateEvent(wrong_aggregate_artifact, aggregate_version_after + 1, 3, DateTimeOffset.UtcNow, event_source_id, correlation_id, microservice_id, tenant_id, cause, event_b_artifact, "wrong");
        };

        Because of = () => exception = Catch.Exception(() =>
        {
            events = new CommittedAggregateEvents(event_source_id, aggregate_artifact.Id, aggregate_version_after + 1, new[] { event_one, event_two, event_three, wrong_aggregate_event });
        });
