...
namespace Dolittle.Runtime.EventHorizon.Consumer.for_EventHorizonEventProcessor.when_processing
{
    public class an_event : given.all_dependencies
    {
        ...
        Establish context = () => processor = new EventProcessor(subscription_id, event_horizon_events_writer.Object, Moq.Mock.Of<ILogger>());

        Because of = () => result = processor.Process(@event, partition, default).GetAwaiter().GetResult();

        It should_write_event = () => event_horizon_events_writer.Verify(_ => _.Write(@event, subscription_id.ScopeId, Moq.It.IsAny<CancellationToken>()), Moq.Times.Once);
