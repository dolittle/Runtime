...
namespace Dolittle.Runtime.Aggregates.for_AggregateRoots.when_registering
{
    ...
    {
        Establish context = () => aggregate_roots.Register(AnAggregateRoot with { Alias = "Some other alias" });
        Because of = () => aggregate_roots.Register(AnAggregateRoot);

        It should_only_have_the_last_registered_aggregate_root = () => aggregate_roots.All.ShouldContainOnly(AnAggregateRoot);
    }
}
