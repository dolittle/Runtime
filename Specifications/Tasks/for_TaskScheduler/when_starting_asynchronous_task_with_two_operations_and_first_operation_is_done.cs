...
namespace doLittle.Specs.Tasks.for_TaskScheduler
{
    ...
    public class when_starting_asynchronous_task_with_two_operations_and_first_operation_is_done : given.a_task_scheduler
    {
        ...
        Establish context = () =>
        {
            task = new TaskWithTwoOperations(true) { CurrentOperation = 1 };
        ...
        It should_start_both_operations = () => scheduler_mock.Verify(s => s.Start<Task>(Moq.It.IsAny<Action<Task>>(), task, Moq.It.IsAny<Action<Task>>()), Moq.Times.Exactly(1));
        ...
        It should_schedule_second_operation = () => task.SecondOperationCalled.ShouldBeTrue();
