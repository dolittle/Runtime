...
namespace Dolittle.Runtime.Events.Processing.for_RemoteFilterProcessor.when_processing
{
    public class and_filtering_needs_retry : given.all_dependencies
    {
        ...
        Establish context = () => processing_processor = new RemoteFilterProcessor(event_processor_id, target_stream_id, Processing.given.a_remote_filter_service(new RetryFilteringResult(retry_timeout)), event_to_stream_writer_mock.Object);
        ...
        It should_write_the_event_to_stream = () => event_to_stream_writer_mock.Verify(_ => _.Write(Moq.It.IsAny<CommittedEventEnvelope>(), Moq.It.IsAny<StreamId>()), Moq.Times.Never());
    }
}