...
namespace Dolittle.Runtime.Events.Processing.Streams.for_FailingPartitions.when_catching_up.and_there_are_multiple_failing_partitions.that_should_be_retried
{
    public class and_must_retry_processing_twice_before_failing : given.all_dependencies
    {
        ...
        It should_have_new_first_failing_partition_reason = () => result.FailingPartitions[first_failing_partition_id].Reason.ShouldEqual(last_failure_reason);
        It should_have_new_second_failing_partition_reason = () => result.FailingPartitions[second_failing_partition_id].Reason.ShouldEqual(last_failure_reason);
        It should_have_retried_processing_first_failing_partition_twice = () => event_processor.Verify(_ => _.Process(committed_events[0], first_failing_partition_id, Moq.It.IsAny<string>(), Moq.It.IsAny<uint>(), Moq.It.IsAny<CancellationToken>()), Moq.Times.Exactly(2));
        It should_have_retried_processing_second_failing_partition_twice = () => event_processor.Verify(_ => _.Process(committed_events[1], second_failing_partition_id, Moq.It.IsAny<string>(), Moq.It.IsAny<uint>(), Moq.It.IsAny<CancellationToken>()), Moq.Times.Exactly(2));
        It should_never_retry_processing_first_failing_partition = () => result.FailingPartitions[first_failing_partition_id].RetryTime.ShouldEqual(DateTimeOffset.MaxValue);
        It should_never_retry_processing_second_failing_partition = () => result.FailingPartitions[second_failing_partition_id].RetryTime.ShouldEqual(DateTimeOffset.MaxValue);
    }
}