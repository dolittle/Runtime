...
{
    public class and_processing_should_retry : given.all_dependencies
    {
        ...
        Establish context = () =>
        {
            ...
            event_processor
                ...
                .Returns(Task.FromResult<IProcessingResult>(new RetryProcessingResult(0, failure_reason)));
        };
        ...
        Because of = () => stream_processor_states.ProcessEventAndChangeStateFor(
            ...
            stream_event,
            new StreamProcessorState(0, new Dictionary<PartitionId, FailingPartitionState>())).GetAwaiter().GetResult();
        ...
        It should_add_failing_partition = () => failing_partitions.Verify(_ => _.AddFailingPartitionFor(stream_processor_id, partition, 0, Moq.It.IsInRange(DateTimeOffset.Now.AddMilliseconds(-100), DateTimeOffset.Now.AddMilliseconds(100), Moq.Range.Inclusive), failure_reason, Moq.It.IsAny<CancellationToken>()));
