...
public class and_processing_fails_at_second_event_in_both_partitions : given.all_dependencies
{
    ...
    It should_have_persisted_correct_position_on_the_first_failing_partition = () => current_stream_processor_state.FailingPartitions[first_partition_id].Position.StreamPosition.ShouldEqual(new StreamPosition(2));
    It should_have_persisted_correct_position_on_the_second_failing_partition = () => current_stream_processor_state.FailingPartitions[second_partition_id].Position.StreamPosition.ShouldEqual(new StreamPosition(3));
