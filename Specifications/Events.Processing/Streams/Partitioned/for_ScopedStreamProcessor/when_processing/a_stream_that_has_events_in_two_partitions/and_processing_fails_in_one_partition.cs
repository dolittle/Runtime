...
namespace Dolittle.Runtime.Events.Processing.Streams.Partitioned.for_ScopedStreamProcessor.when_processing.a_stream_that_has_events_in_two_partitions
{
    public class and_processing_fails_in_one_partition : given.all_dependencies
    {
        ...
        Establish context = () =>
        {
            ...
            third_event = new StreamEvent(committed_events.single(), 2u, Guid.NewGuid(), second_partition_id);
            fourth_event = new StreamEvent(committed_events.single(), 3u, Guid.NewGuid(), second_partition_id);
            event_processor
                .Setup(_ => _.Process(Moq.It.IsAny<CommittedEvent>(), second_partition_id, Moq.It.IsAny<CancellationToken>()))
                .Returns(Task.FromResult<IProcessingResult>(new SuccessfulProcessing()));
            event_processor
                .Setup(_ => _.Process(Moq.It.IsAny<CommittedEvent>(), first_partition_id, Moq.It.IsAny<CancellationToken>()))
        ...
        Because of = () =>
        {
            stream_processor.Start(CancellationToken.None).GetAwaiter().GetResult();
        };

        It should_process_event_in_first_partition_once = () => event_processor.Verify(_ => _.Process(Moq.It.IsAny<CommittedEvent>(), first_partition_id, Moq.It.IsAny<CancellationToken>()), Moq.Times.Once);
        It should_process_three_events_in_second_partition = () => event_processor.Verify(_ => _.Process(Moq.It.IsAny<CommittedEvent>(), second_partition_id, Moq.It.IsAny<CancellationToken>()), Moq.Times.Exactly(3));
        ...
        It should_process_third_event = () => event_processor.Verify(_ => _.Process(third_event.Event, second_partition_id, Moq.It.IsAny<CancellationToken>()), Moq.Times.Once);
        ...
        It should_have_persisted_state_with_one_failing_partition = () => current_state.FailingPartitions.Count.ShouldEqual(1);
