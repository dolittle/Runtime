...
public class and_processing_fails_at_second_event_in_both_partitions : all_dependencies
{
    ...
    Establish context = () =>
    {
        ...
        event_processor
            .Setup(_ => _.Process(first_event.Event, Moq.It.IsAny<PartitionId>(), Moq.It.IsAny<CancellationToken>()))
            .Returns(Task.FromResult<IProcessingResult>(new FailedProcessing(reason)));
        event_processor
            .Setup(_ => _.Process(second_event.Event, Moq.It.IsAny<PartitionId>(), Moq.It.IsAny<CancellationToken>()))
