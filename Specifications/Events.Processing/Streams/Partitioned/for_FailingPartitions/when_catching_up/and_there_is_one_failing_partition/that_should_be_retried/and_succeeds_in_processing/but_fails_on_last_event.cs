...
public class but_fails_on_last_event : given.all_dependencies
{
    ...
    It should_have_the_correct_position_on_failing_partition = () => failing_partition(failing_partition_id).Position.Should().Be(new StreamPosition(initial_failing_partition_position + 2));
    It should_have_new_failing_partition_reason = () => failing_partition(failing_partition_id).Reason.Should().Be(new_failure_reason);
