...
public class and_must_retry_processing_twice_before_failing : given.all_dependencies
{
    ...
    It should_have_new_first_failing_partition_reason = () => failing_partition(first_failing_partition_id).Reason.Should().Be(last_failure_reason);
    It should_have_new_second_failing_partition_reason = () => failing_partition(second_failing_partition_id).Reason.Should().Be(last_failure_reason);
    ...
    It should_never_retry_processing_first_failing_partition = () => failing_partition(first_failing_partition_id).RetryTime.Should().Be(DateTimeOffset.MaxValue);
    It should_never_retry_processing_second_failing_partition = () => failing_partition(second_failing_partition_id).RetryTime.Should().Be(DateTimeOffset.MaxValue);
