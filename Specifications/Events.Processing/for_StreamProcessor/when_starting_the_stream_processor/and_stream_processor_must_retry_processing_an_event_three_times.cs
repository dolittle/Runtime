...
namespace Dolittle.Runtime.Events.Processing.for_StreamProcessor.when_starting_the_stream_processor
{
    public class and_stream_processor_must_retry_processing_an_event_three_times : given.all_dependencies
    {
        ...
        It Should_have_persisted_final_state = async () => (await stream_processor_state_repository.Get(stream_processor.Key).ConfigureAwait(false)).ShouldEqual(new StreamProcessorState(StreamProcessingState.Stopping, 0));
    }
}