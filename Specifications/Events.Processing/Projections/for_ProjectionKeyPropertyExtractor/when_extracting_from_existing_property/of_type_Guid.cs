...
public class of_type_Guid : given.all_dependencies
{
    static bool result;
    ...
    Because of = () => result = extractor.TryExtract(JsonSerializer.Serialize(json_object), nameof(json_object.a_guid_property), out key);

    It should_return_true = () => result.Should().BeTrue();
    It should_set_key_correct_value = () => key.Value.Should().Be(json_object.a_guid_property.ToString());
}