...
{
    /// <summary>
    /// Represents an implementation of <see cref="AbstractFilterOnWellKnownStreamsValidator" /> that can validate filters that are based off the event log stream.
    /// </summary>
    public class EventLogFilterValidator : AbstractFilterOnWellKnownStreamsValidator
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="EventLogFilterValidator"/> class.
        /// </summary>
        ...
        public EventLogFilterValidator(
            IFetchEventTypesFromStreams eventTypesFromStreams,
            EventStoreConnection connection,
            ILogger logger)
            : base(new StreamId[] { StreamId.AllStreamId }, eventTypesFromStreams, connection)
        {
        ...
        public override async Task Validate(StreamId sourceStream, StreamId targetStream, AbstractFilterProcessor filter, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var committedEvents = await _connection.EventLog
                    .Find(Builders<Event>.Filter.Lte(_ => _.EventLogVersion, lastEventLogVersion))
