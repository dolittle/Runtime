...
namespace Dolittle.Runtime.Events.Processing.Filters
{
    ...
    public class IllegalFilterTransformation : Exception
    {
        ...
        public IllegalFilterTransformation(StreamId targetStream, StreamId sourceStream)
            : base($"Cannot register filter with target stream '{targetStream}' and source stream '{sourceStream}' because it produces a different stream compared to the previously registered filter. Change the target Stream Id of the new filter to create a new stream.")
        {
        }
    }
}