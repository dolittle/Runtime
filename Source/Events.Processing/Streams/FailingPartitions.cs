...
namespace Dolittle.Runtime.Events.Processing.Streams
{
    ...
    public class FailingPartitions : IFailingPartitions
    {
        ...
        /// <inheritdoc/>
        public Task<StreamProcessorState> AddFailingPartitionFor(StreamProcessorId streamProcessorId, PartitionId partition, StreamPosition position, DateTimeOffset retryTime, string reason, CancellationToken cancellationToken)
        {
            ...
            foreach (var kvp in failingPartitionsList)
            {
                ...
                if (ShouldRetryProcessing(failingPartitionState))
                {
                    ...
                    while (ShouldProcessNextEventInPartition(nextPosition, streamProcessorState.Position))
                    {
                        ...
                        {
                            (streamProcessorState, failingPartitionState) = await SetFailingPartitionState(streamProcessorId, partition, failingPartitionState.ProcessingAttempts + 1, processingResult.RetryTimeout, processingResult.FailureReason, nextPosition, cancellationToken).ConfigureAwait(false);
                        }
                        else
                        {
                            (streamProcessorState, failingPartitionState) = await SetFailingPartitionState(streamProcessorId, partition, failingPartitionState.ProcessingAttempts + 1, DateTimeOffset.MaxValue, processingResult.FailureReason, nextPosition, cancellationToken).ConfigureAwait(false);
                        }
        ...
        Task<(StreamProcessorState, FailingPartitionState)> SetFailingPartitionState(StreamProcessorId streamProcessorId, PartitionId partitionId, uint processingAttempts, TimeSpan retryTimeout, string reason, StreamPosition position, CancellationToken cancellationToken) => SetFailingPartitionState(streamProcessorId, partitionId, processingAttempts, DateTimeOffset.UtcNow.Add(retryTimeout), reason, position, cancellationToken);

        async Task<(StreamProcessorState, FailingPartitionState)> SetFailingPartitionState(StreamProcessorId streamProcessorId, PartitionId partitionId, uint processingAttempts, DateTimeOffset retryTime, string reason, StreamPosition position, CancellationToken cancellationToken)
        {
            ...
            var newFailingPartitionState = new FailingPartitionState { Position = position, RetryTime = retryTime, Reason = reason, ProcessingAttempts = processingAttempts };
            var streamProcessorState = await _streamProcessorStates.SetFailingPartitionState(
                streamProcessorId,
                partitionId,
                newFailingPartitionState,
