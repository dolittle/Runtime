...
namespace Dolittle.Runtime.Events.Processing
{
    ...
    public class ProcessorFailure
    {
        ...
        public ProcessorFailure(string reason)
        {
            Reason = reason;
        }
        ...
        public ProcessorFailure(string reason, bool retry, TimeSpan retryTimeout)
        {
            Reason = reason;
            Retry = retry;
        ...
        public static implicit operator ProcessorFailure(grpc.ProcessorFailure failure)
        {
            ...
            if (failure.Retry) return new ProcessorFailure(failure.Reason, failure.Retry, failure.RetryTimeout.ToTimeSpan());
            else return new ProcessorFailure(failure.Reason);
        }
    }
}