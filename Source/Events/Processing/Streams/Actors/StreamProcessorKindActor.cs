...
/// <summary>
/// Represents a system for working with all the <see cref="StreamProcessorActor" /> registered for <see cref="ITenants.All" />.
/// </summary>
public class StreamProcessorKindActor : IDisposable, IActor
{
    ...
    readonly Func<TenantId, IServiceProvider> _tenantedServiceProvider;
    readonly IMetricsCollector _metrics;
    readonly ILogger _logger;
    readonly ExecutionContext _executionContext;
    readonly CancellationTokenSource _stopAllScopedStreamProcessorsTokenSource;
    readonly Dictionary<TenantId, StreamProcessorActor> _streamProcessors = new();
    bool _started;
    bool _disposed;

    /// <summary>
    /// Initializes a new instance of the <see cref="StreamProcessorKindActor"/> class.
    /// </summary>
    ...
    /// <param name="eventProcessorKind"></param>
    ...
    public StreamProcessorKindActor(
        ...
        CancellationToken cancellationToken, Func<TenantId, IServiceProvider> tenantedServiceProvider)
    {
        ...
        _tenantedServiceProvider = tenantedServiceProvider;
    ...
    async Task OnStarted(IContext context)
    {
        ...
        if (_stopAllScopedStreamProcessorsTokenSource.Token.IsCancellationRequested)
        {
    ...
    async Task InitTenant(TenantId tenant)
    {
        ...
        var serviceProvider = _tenantedServiceProvider.Invoke(tenant);
        ActivatorUtilities.CreateInstance<StreamProcessorActor>(serviceProvider, _sp)
    ...
    /// <returns>The <see cref="Task"/> that, when resolved, returns a <see cref="Try{TResult}"/> with the <see cref="StreamPosition"/> it was set to.</returns>
    public Task<Try<StreamPosition>> PerformActionAndSetToPosition(TenantId tenant, StreamPosition position,
        Func<TenantId, CancellationToken, Task<Try>> action)
    {
    ...
    /// <param name="action">The action to perform before setting the position.</param>
    /// <returns>The <see cref="Task"/> that, when resolved, returns a <see cref="Dictionary{TKey,TValue}"/> with a <see cref="Try{TResult}"/> with the <see cref="StreamPosition"/> it was set to for each <see cref="TenantId"/>.</returns>
    public async Task<IDictionary<TenantId, Try<StreamPosition>>> PerformActionAndSetToInitialPositionForAllTenants(
        Func<TenantId, CancellationToken, Task<Try>> action)
    {
